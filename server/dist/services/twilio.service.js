...
class TwilioService {
    ...
    async notifyMatchingFreelancers(job) {
        if (!this.client)
            return;
        try {
            ...
            const profiles = await Profile_model_1.default.find(query)
            ...
            await Promise.all(eligibleProfiles.map((p) => {
                if (p.phoneNumber) {
