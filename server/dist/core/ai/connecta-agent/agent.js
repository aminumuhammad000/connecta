...
export class ConnectaAgent {
    ...
    async initialize() {
        ...
        let modelName = aiConfig.model || "gemini-1.5-flash-8b";
    ...
    async process(input) {
        ...
        if (!this.model) {
            ...
            if (!this.model) {
                return this.createResponse("I'm having trouble connecting to my brain (AI Model). Please contact support.", null, false, startTime);
            }
        }
        try {
...
Try: "Find gigs for me" or "Show my profile"`;
            ...
            const chain = RunnableSequence.from([
                ...
                async (rawOutput) => {
                    ...
                    const relatedTasks = getRelatedTasks(validatedOutput.tool);
                    const formattedTasks = formatRelatedTasks(relatedTasks);
            ...
            return this.createResponse("I encountered an error processing your request with Gemini. Please check your API configuration.", null, false, startTime);
        }
    }
