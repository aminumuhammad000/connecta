...
export const approveProposal = async (req, res) => {
    try {
        ...
        try {
            ...
            await notifyProposalAccepted(proposal.freelancerId, client ? `${client.firstName} ${client.lastName}` : 'Client', job.title, job._id);
        }
        catch (err) {
        ...
        try {
            ...
            const freelancerName = freelancerUser ? `${freelancerUser.firstName || ''} ${freelancerUser.lastName || ''}`.trim() : 'a worker';
            createFeedPost({
