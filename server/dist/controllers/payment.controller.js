    ...
    try {
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                authorizationUrl: vtstackResponse.data.authorization_url || vtstackResponse.data.link,
 ...
 */
export const getOrCreateVirtualAccount = async (req, res) => {
    try {
        const userId = req.user?.id || req.user?._id || req.user?.userId;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        ...
        if (phoneToUse.length > 11)
            phoneToUse = phoneToUse.slice(-11);
 ...
 */
export const resolveFlutterwaveAccount = async (req, res) => {
    try {
        const { accountNumber, bankCode } = req.body;
        if (!accountNumber || !bankCode) {
