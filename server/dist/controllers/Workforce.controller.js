...
// DASHBOARD & SETTINGS
// ============================================
export const getPublicWorkforceInfo = async (req, res) => {
    try {
...
export const getWorkerMeData = async (req, res) => {
    try {
...
export const saveSettings = async (req, res) => {
    try {
        ...
        if (workerCountRange !== undefined)
            settings.workerCountRange = workerCountRange;
        if (hasExistingWorkers !== undefined)
            settings.hasExistingWorkers = hasExistingWorkers;
        if (requireLocationForAttendance !== undefined)
            settings.requireLocationForAttendance = requireLocationForAttendance;
        if (allowSelfCheckIn !== undefined)
            settings.allowSelfCheckIn = allowSelfCheckIn;
...
export const bulkImportWorkers = async (req, res) => {
    try {
        const userId = req.user._id || req.user.id;
        ...
        for (const w of workers) {
            try {
                ...
                await WorkforceMember.create({
                    ...
                    role: w.role.trim(),
                    skills: w.skills ? String(w.skills).split(',').map((s) => s.trim()) : [],
...
// ============================================
export const getWorkforceJobs = async (req, res) => {
    try {
        ...
        const workforceEmployerIds = Array.from(new Set([
...
export const updateWorkerPayoutStatus = async (req, res) => {
    try {
...
export const fundPayrollWallet = async (req, res) => {
    try {
        const userId = req.user._id || req.user.id;
