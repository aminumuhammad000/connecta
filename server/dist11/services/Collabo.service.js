...
class CollaboService {
    ...
    async startWork(projectId, clientId) {
        ...
        const project = await CollaboProject.findByIdAndUpdate(projectId, { status: 'in_progress' }, { new: true }).populate('clientId', 'firstName lastName email');
        if (!project)
            throw new Error("Project not found");
        ...
        for (const role of roles) {
            if (role.freelancerId) {
                ...
                try {
                    ...
                    await sendEmail(freelancer.email, `Work Started: ${project.title}`, emailHtml);
                }
                catch (emailError) {
    ...
    async getClientProjectsWithRoles(clientId) {
    ...
    async getFreelancerProjects(freelancerId) {
    ...
    async acceptRole(roleId, freelancerId) {
        // Transactions removed for standalone MongoDB compatibility
        try {
            const role = await ProjectRole.findOneAndUpdate({ _id: roleId, status: 'open' }, { freelancerId, status: 'filled' }, { new: true }).populate('freelancerId', 'firstName lastName email');
            if (!role)
            ...
            await CollaboWorkspace.findOneAndUpdate({ projectId: role.projectId }, { $addToSet: { "channels.$[elem].roleIds": role._id } }, { arrayFilters: [{ "elem.name": "General" }] });
            ...
            try {
                ...
                await sendEmail(client.email, `New Team Member: ${project.title}`, emailHtml);
            }
            catch (emailError) {
    ...
    async removeFromRole(roleId, clientId) {
        try {
            ...
            const removedFreelancer = role.freelancerId;
            // Remove from role
            role.freelancerId = undefined;
            role.status = 'open';
            await role.save();
            // Remove from workspace channels
            await CollaboWorkspace.findOneAndUpdate({ projectId: role.projectId }, { $pull: { "channels.$[].roleIds": role._id } });
            ...
            if (removedFreelancer) {
                ...
                try {
                    ...
                    await sendEmail(removedFreelancer.email, `Removed from ${project.title}`, emailHtml);
                }
                catch (emailError) {
    ...
    async inviteToRole(roleId, freelancerId, clientId) {
        try {
            ...
            const projectClientId = project.clientId._id
            ...
            await Notification.create({
                ...
                message: `${project.clientId.firstName} invited you to join "${project.title}" as ${role.title}`,
                relatedId: roleId,
                relatedType: 'project',
            ...
            try {
                const emailHtml = getBaseTemplate({
                    ...
                    content: `
                        <p>Hi ${freelancer.firstName},</p>
                        <p><strong>${project.clientId.firstName} ${project.clientId.lastName}</strong> has invited you to join their team!</p>
                ...
                await sendEmail(freelancer.email, `Team Invitation: ${project.title}`, emailHtml);
            }
            catch (emailError) {
    ...
    async addRole(projectId, clientId, roleData) {
