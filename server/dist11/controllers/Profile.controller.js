 ...
 */
const formatProfileResponse = (profile, reviews = [], jobs = []) => {
...
export const getMyProfile = async (req, res) => {
    try {
        ...
        let profile = await Profile.findOne({ user: userId }).populate('user', 'firstName lastName email profileImage userType isPremium subscriptionTier subscriptionStatus premiumExpiryDate averageRating totalReviews jobSuccessScore badges performanceMetrics');
        ...
        if (!profile) {
            ...
            // Populate user data
            profile = await Profile.findById(profile._id).populate('user', 'firstName lastName email profileImage userType isPremium subscriptionTier subscriptionStatus premiumExpiryDate');
        }
...
export const getProfileById = async (req, res) => {
    try {
        const profile = await Profile.findById(req.params.id).populate("user", "firstName lastName email profileImage userType isPremium subscriptionTier subscriptionStatus premiumExpiryDate averageRating totalReviews jobSuccessScore badges performanceMetrics");
        if (!profile) {
...
export const getProfileByUserId = async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.params.userId }).populate("user", "firstName lastName email profileImage userType isPremium subscriptionTier subscriptionStatus premiumExpiryDate averageRating totalReviews jobSuccessScore badges performanceMetrics");
        if (!profile) {
            return res.status(404).json({ message: "Profile not found" });
        }
        // Fetch reviews
...
export const updateProfile = async (req, res) => {
    try {
        ...
        const updatedProfile = await Profile.findByIdAndUpdate(req.params.id, { phoneNumber, location, resume, education, languages, employment }, { new: true, runValidators: true });
        if (!updatedProfile) {
            return res.status(404).json({ message: "Profile not found" });
        }
...
export const updateMyProfile = async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        const { phoneNumber, location, country, city, timezone, preferredLanguage, companyName, website, bio, avatar, skills, education, languages, employment, resume, portfolio, remoteWorkType, minimumSalary, workLocationPreferences, jobTitle, jobCategories, yearsOfExperience, engagementTypes, jobNotificationFrequency } = req.body;
        ...
        if (remoteWorkType !== undefined)
            updateData.remoteWorkType = remoteWorkType;
        if (minimumSalary !== undefined)
            updateData.minimumSalary = minimumSalary;
        if (workLocationPreferences !== undefined)
            updateData.workLocationPreferences = workLocationPreferences;
        if (jobTitle !== undefined)
            updateData.jobTitle = jobTitle;
        if (jobCategories !== undefined)
            updateData.jobCategories = jobCategories;
        if (yearsOfExperience !== undefined)
            updateData.yearsOfExperience = yearsOfExperience;
        if (engagementTypes !== undefined)
            updateData.engagementTypes = engagementTypes;
        if (jobNotificationFrequency !== undefined)
            updateData.jobNotificationFrequency = jobNotificationFrequency;
