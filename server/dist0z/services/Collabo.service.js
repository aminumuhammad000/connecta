...
class CollaboService {
    ...
    async createCollaboProject(clientId, data) {
        ...
        try {
            ...
            const createdRoles = await ProjectRole_model_1.default.insertMany(roleDocs, session ? { session } : {});
    ...
    async acceptRole(roleId, freelancerId) {
        ...
        try {
            ...
            const role = await ProjectRole_model_1.default.findOneAndUpdate({ _id: roleId, status: 'open' }, { freelancerId, status: 'filled' }, { new: true, ...(session ? { session } : {}) });
            if (!role)
                throw new Error("Role not found or already filled");
            // Add to workspace channel (General)
            await CollaboWorkspace_model_1.default.findOneAndUpdate({ projectId: role.projectId }, { $addToSet: { "channels.$[elem].roleIds": role._id } }, { arrayFilters: [{ "elem.name": "General" }], ...(session ? { session } : {}) });
            if (useTransaction && session) {
                await session.commitTransaction();
            }
            return role;
