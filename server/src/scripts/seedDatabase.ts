...
const seedDatabase = async () => {
  try {
    ...
    const jobsData = clients.flatMap((client, clientIndex) =>
      Array.from({ length: 2 }, (_, jobIndex) => {
        ...
        return {
          ...
          locationType: ['remote', 'onsite', 'hybrid'][jobIndex % 3] as any,
          jobType: ['full-time', 'part-time', 'contract', 'freelance'][jobIndex % 4] as any,
          ...
          status: ['active', 'closed', 'draft'][jobIndex % 3] as any,
    ...
    const proposalsData = jobs.flatMap((job, jobIndex) =>
      freelancers.slice(0, 2).map((freelancer, freelancerIndex) => ({
        jobId: job._id,
        ...
        type: freelancerIndex % 2 === 0 ? 'recommendation' : 'referral' as any,
        status: ['pending', 'accepted', 'declined'][freelancerIndex % 3] as any,
        level: ['entry', 'intermediate', 'expert'][freelancerIndex % 3] as any,
        priceType: freelancerIndex % 2 === 0 ? 'fixed' : 'hourly' as any,
    ...
    const projectsData = acceptedProposals.map((proposal, index) => {
      ...
      return {
        ...
        status: ['ongoing', 'completed', 'cancelled'][index % 3] as any,
    ...
    const contractsData = projects.map((project, index) => ({
      ...
      contractType: project.budget.type === 'fixed' ? 'fixed_price' : 'hourly' as any,
      terms: [
        {
          title: 'Scope of Work',
      ...
      status: ['draft', 'pending_signatures', 'active'][index % 3] as any,
    ...
    const paymentsData = projects.map((project, index) => ({
      ...
      status: ['pending', 'completed', 'failed'][index % 3] as any,
      paymentMethod: 'paystack' as const,
      paymentType: 'milestone' as const,
      gatewayReference: `PAY-${Date.now()}-${index}`,
      escrowStatus: ['held', 'released', 'none'][index % 3] as any,
    ...
    const subscriptionsData = premiumUsers.map((user, index) => ({
      ...
      amount: 5000,
      currency: 'NGN',
      status: ['active', 'expired', 'cancelled'][index % 3] as any,
    ...
    const notificationsData = users.flatMap((user, userIndex) =>
      Array.from({ length: 2 }, (_, notifIndex) => ({
        userId: user._id,
        ...
        relatedType: ['proposal', 'proposal', 'project', 'payment', 'message', 'review'][notifIndex % 6] as any,
        icon: ['📋', '✅', '🚀', '💰', '💬', '⭐'][notifIndex % 6],
        priority: ['low', 'medium', 'high'][notifIndex % 3] as any,
    ...
    const withdrawalsData = freelancers.slice(0, 3).map((freelancer, index) => {
      ...
      return {
        ...
        status: ['pending', 'processing', 'completed'][index % 3] as any,
      };
    });
