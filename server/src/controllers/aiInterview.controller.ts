...
async function generateContextualQuestions(job: any, freelancer: any, profile: any, proposal: any) {
...
export const startAiInterview = async (req: Request, res: Response) => {
  try {
    ...
    if (freelancer?._id?.toString() !== userId?.toString() && userId?.toString() !== (req as any).user?.id) {
...
export const submitAnswer = async (req: Request, res: Response) => {
  try {
    ...
    if (existingIndex >= 0) {
      ...
      if (audioUrl) interview.answers[existingIndex].audioUrl = audioUrl;
...
export const completeAiInterview = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
export const getInterviewByProposalId = async (req: Request, res: Response) => {
  try {
...
export const streamElevenLabsSpeech = async (req: Request, res: Response) => {
  try {
