...
export const chatWithAI = async (req: Request, res: Response) => {
  try {
    ...
    const formatBudget = (rawBudget: number, jobCurrency?: string) => {
      let b = rawBudget || 500;
      if (isNairaUser) {
    ...
    if (isSubmitAction) {
      ...
      if (targetJob) {
        ...
        if (!proposalDoc) {
          const clientName = (targetJob.clientId as any)?.companyName || `${(targetJob.clientId as any)?.firstName || 'Hiring'} Manager`;
    ...
    if (isJobQuery) {
      ...
      if (activeDbJobs && activeDbJobs.length > 0) {
        matchedJobsData = activeDbJobs.map((job: any) => {
...
export const parseCvWithAI = async (req: Request, res: Response) => {
  try {
    let rawText = '';
