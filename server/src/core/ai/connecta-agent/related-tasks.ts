 ...
 */
export const RELATED_TASKS_MAP: Record<string, string[]> = {
    // Profile Tools
...
export function getRelatedTasks(
    ...
    const suggestions = RELATED_TASKS_MAP[toolName] || RELATED_TASKS_MAP.default;
 ...
 */
export function formatRelatedTasks(tasks: string[]): string {
 ...
 */
export function getQuickActions(toolName: string): Array<{
    text: string;
    action: string;
}> {
    const actionMap: Record<string, Array<{ text: string; action: string }>> = {
        get_profile_details_tool: [
