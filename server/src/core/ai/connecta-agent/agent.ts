...
export class ConnectaAgent {
  ...
  private async loadUserContext(): Promise<void> {
    ...
    // If no auth token, try a public fallback by listing profiles and matching userId
    if (!this.config.authToken) {
      try {
        ...
        if (profile) {
          ...
          this.saveMemory();
          console.log(`✅ User context loaded (public): ${name ?? "Unknown"} (${userType ?? "unknown"})`);
          return;
        }
      } catch (err) {
        console.info("ℹ️ Public profile lookup unavailable or failed; continuing without profile.");
      }
    ...
    try {
      const url = `${this.config.apiBaseUrl}/api/profiles/me`;
      const headers = this.config.authToken ? { Authorization: `Bearer ${this.config.authToken}` } : {};
      ...
      this.saveMemory();
      console.log(`✅ User context loaded: ${name ?? "Unknown"} (${userType ?? "unknown"})`);
    } catch (e) {
