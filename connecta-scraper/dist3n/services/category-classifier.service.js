...
exports.CategoryClassifierService = void 0;
...
class CategoryClassifierService {
    ...
    classify(gig) {
        ...
        if (maxScore > 0) {
            logger_1.logger.debug(`📂 Classified "${gig.title}" as ${bestCategory}${bestNiche ? ` > ${bestNiche}` : ""} (score: ${maxScore})`);
        }
        else {
            logger_1.logger.debug(`📂 No clear category for "${gig.title}", defaulting to "Other"`);
        }
...
exports.CategoryClassifierService = CategoryClassifierService;
