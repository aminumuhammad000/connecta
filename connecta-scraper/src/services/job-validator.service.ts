...
export class JobValidatorService {
     ...
     */
    validate(gig: ExternalGig): { isValid: boolean; errors: string[] } {
        const errors: string[] = [];
        ...
        if (!isValid) {
            logger.warn(`❌ Job validation failed for "${gig.title}": ${errors.join(", ")}`);
        } else {
            logger.debug(`✅ Job validation passed for "${gig.title}"`);
        }
     ...
     */
    validateBatch(gigs: ExternalGig[]): {
        valid: ExternalGig[];
        invalid: Array<{ gig: ExternalGig; errors: string[] }>;
    } {
        const valid: ExternalGig[] = [];
        const invalid: Array<{ gig: ExternalGig; errors: string[] }> = [];
        ...
        logger.info(`✅ Validated ${valid.length}/${gigs.length} jobs successfully`);
        if (invalid.length > 0) {
            logger.warn(`⚠️ ${invalid.length} jobs failed validation`);
        }
