          ...
          {loading ? (
            ...
            filteredNotifications.map((notif) => {
              ...
              return (
                <div
                  key={notif._id}
                  className={`flex items-start gap-4 p-4 md:p-6 relative transition-colors ${isUnread ? 'bg-primary/5 hover:bg-primary/10' : 'hover:bg-background-light dark:hover:bg-background-dark/50'
                    }`}
                >
                  {isUnread && <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-l-xl" />}

                  {/* Icon */}
                  <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${isUnread ? 'bg-white dark:bg-background-dark' : 'bg-background-light dark:bg-background-dark'
                    }`}>
